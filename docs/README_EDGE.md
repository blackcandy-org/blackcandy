...
| BACKGROUND_JOB_SUPERVISOR_MODE | "async" | The mode used to run background jobs. By default Black Candy runs them in "async" mode to keep memory usage low by processing jobs in the same process as the supervisor. Set this to "fork" to run workers and dispatchers in separate processes if you need better performance, but it will cost more memory usage.                                                                                              |
